...
from .serializers import TaskSerializer,CategorySerializer,AttachmmentSerializer
import json

# Create your views here.
#views related to Taskapi
...
#views related to the categoryapi
...
# Views  for both task and category
...
#views for the task and attachments  
class Task_Attachmentsapi(APIView):
    def get(self,request,pk=None):
        if pk is not None:
            try:
                resource=Task.objects.filter(id=pk)
                serializer=TaskSerializer(resource,many=True)
                serialized_data =serializer.data
                # print(serialized_data)
                processed_data=[]
                ...
                #    finding the category name based on the id
                   category=Task_category.objects.get(id=task_category_id)
                   attachments = Task_Attachments.objects.filter(task_id=pk)
                   serializer=AttachmmentSerializer(attachments,many=True)
                   serialized_attachments=serializer.data
                   processed_attachments =[]
                   for task_attachments in serialized_attachments:
                       
                       Attachments={
                       'Description':task_attachments['description'],
                       'Attachments':task_attachments['attachments']
                       }
                       processed_attachments.append(Attachments)
...
class Attachmentsapi(APIView):
      def post(self,request):
        ...
        id=request.data.get('id')
        serializer=AttachmmentSerializer(data=request.data)
        
        
        if(Task_Attachments.objects.filter(id=id).exists()):
            error_message="Entered id is already taken please enter new id"
            return Response({"Data error":error_message})
        else:
        # print(id)
